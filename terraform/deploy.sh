...
# Function to recreate infrastructure (destroy and deploy)
recreate_infrastructure() {
    print_warning "This will DESTROY and RECREATE ALL infrastructure resources!"
    print_warning "This ensures a completely fresh deployment but will cause downtime."
    echo
    read -p "Are you sure you want to recreate the infrastructure? (y/N): " -n 1 -r
    echo
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        # Then deploy fresh infrastructure
        print_status "Planning fresh infrastructure deployment..."
        terraform plan -out=tfplan

        echo
        read -p "Proceed with fresh infrastructure deployment? (y/N): " -n 1 -r
        echo

        if [[ $REPLY =~ ^[Yy]$ ]]; then
            print_status "Deploying fresh infrastructure..."
            terraform apply -auto-approve tfplan
            print_success "Infrastructure recreated successfully"
        else
            print_error "Fresh deployment cancelled. Infrastructure is now destroyed!"
            exit 1
        fi
    else
        print_warning "Infrastructure recreation cancelled"
        exit 0
    fi
}
...
main() {
    ...
    case "${1:-deploy}" in
        ...
        "help"|"-h"|"--help")
            ...
            echo "  recreate                      RECREATE infrastructure and deploy application (CAUSES DOWNTIME)"
            echo "  recreate-infrastructure-only  RECREATE only the infrastructure (CAUSES DOWNTIME)"
            ...
            echo "  $0 recreate                   # Recreate everything (destroys and rebuilds infrastructure)"
            echo "  $0 recreate-infrastructure-only # Recreate only infrastructure"
            echo "  $0 update-app                 # Update application only"
            echo "  $0 cleanup                    # Destroy everything"
            echo
            echo "IMPORTANT:"
            echo "  - 'recreate' commands will DESTROY and RECREATE infrastructure, causing downtime"
            echo "  - Use 'recreate' when you need to ensure a completely fresh infrastructure deployment"
            echo "  - Use regular 'deploy' for updates without downtime"
            ;;
        *)
